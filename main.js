    ...
    async startPythonProcess() {
        ...
        try {
            ...
            this.pythonProcess.on('error', (err) => {
                ...
                this.log.error(`   sudo apt-get install -y python3 python3-venv`);
                this.log.error(`-----------------------------------------------------------------------------`);
                ...
                this.log.error(`   add "python3" to the "PACKAGES" environment variable.`);
                this.log.error(`   在容器环境变量(Environment)中添加/修改: PACKAGES = python3 (容器会自动在启动时安装)`);
                this.log.error(`-----------------------------------------------------------------------------`);
                ...
                this.log.error(`   - 访问 Python 官网下载并运行安装包，请务必勾选 "Add Python to PATH" 选项！`);
